...
export default function Hero({ onOpenVideo, onOpenCalendar }: HeroProps) {
  return (
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-14 items-center">
          ...
          <div className="lg:col-span-7 flex flex-col items-start space-y-5 sm:space-y-6">
            <div className="animate-hero-badge inline-flex items-center gap-2 text-[#fbbf24]/90 text-[11px] sm:text-sm font-semibold tracking-wider sm:tracking-widest uppercase">
              <span className="w-6 sm:w-8 h-[2px] bg-[#f59e0b]"></span>
              <span>{CHURCH_INFO.slogans.join(" • ")}</span>
            </div>

            <h1 className="animate-hero-title font-headline text-2xl sm:text-4xl lg:text-6xl font-black text-[#faf9f6] tracking-tight leading-tight">
              Experience the{" "}
            ...
            {/* CTAs */}
            <div className="animate-hero-cta pt-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-3.5 w-full sm:w-auto">
              <button
                onClick={onOpenCalendar}
                className="shimmer-sweep inline-flex items-center justify-center gap-2.5 sm:gap-3 px-6 sm:px-8 py-3.5 sm:py-4 rounded-xl bg-gradient-to-r from-[#f59e0b] to-[#fe932c] text-[#070d1a] font-headline text-sm sm:text-base font-bold tracking-wide uppercase hover:shadow-xl hover:shadow-[#f59e0b]/30 hover:scale-[1.03] active:scale-95 transition-all w-full sm:w-auto text-center cursor-pointer"
              >
            ...
            {/* Service times pill */}
            <div className="animate-hero-pill pt-1 flex items-center gap-2 text-[#d3e4fe]/80 text-xs sm:text-sm">
              <span className="material-symbols-outlined text-[#f59e0b] text-base sm:text-lg">schedule</span>
              <span>Sunday Services: <strong>8:00 AM</strong> &amp; <strong>9:15 AM</strong> WAT</span>
            </div>
          </div>

          {/* Right Column: Visual */}
          <div className="lg:col-span-5 relative animate-hero-card">
            <div className="absolute -inset-2 rounded-3xl bg-gradient-to-tr from-[#f59e0b]/35 via-[#0f2042] to-[#fbbf24]/25 blur-lg animate-pulse-aura pointer-events-none"></div>
            <div className="relative bg-[#0f2042] rounded-2xl overflow-hidden border border-[#f59e0b]/30 shadow-2xl animate-float-subtle transition-transform duration-500 hover:scale-[1.02]">
              <div className="relative h-72 sm:h-96 w-full overflow-hidden group">
                ...
                <div className="absolute top-4 right-4 flex items-center gap-2 bg-[#070d1a]/90 border border-[#f59e0b]/40 backdrop-blur-md px-3 py-1.5 rounded-full shadow-md">
                  <span className="w-2.5 h-2.5 rounded-full bg-green-600 animate-pulse"></span>
                  <span className="text-[11px] font-bold text-[#faf9f6] uppercase tracking-wider">GPC Ikeja</span>
                </div>
              </div>
            </div>
          </div>
